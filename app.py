...
def stats_fig(df_all):
    ...
    fig = px.pie(
        ...
        color_discrete_map={'ASSIGNED': '#00FF00', 'NOT ASSIGNED': '#FF0000'},
        template='plotly'
    )
    fig.update_traces(
        textinfo='percent', textfont=dict(size=20)
    )
    fig.update_layout(margin=dict(l=20, r=20, t=0, b=20), showlegend=False)
    return fig
